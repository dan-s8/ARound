...
# print(data.info())  # Index: 94083 entries, 0 to 109689 total 53 columns

data = data.dropna()  # all cols have no missing values after this
data['Event Date'] = pd.to_datetime(data['Event Date'], errors='coerce')
# print(data.info())  # Index: 3892 entries, 0 to 109068 total 53 columns

# Reset index after dropping rows
data = data.reset_index(drop=True)
print(data.info())  # RangeIndex: 3892 entries, 0 to 3891, dtypes: datetime64[ns](1), float64(29), int64(3), object(20)
...
# Index(['sp popularity', 'Median age', 'Number of Shows', 'Ticket Price Min USD', 'Ticket Price Max USD'], dtype='object')

# Evaluate the model
print(f'Root Mean Squared Error (RMSE): {rm5}')  # Root Mean Squared Error (RMSE): 646624.3117811915
# print("Model score:", model.score(X_test[selected_features5], y_test))  # 0.31522249229981136
print(f'R-squared (R2) Score: {r5}')  # R-squared (R2) Score: 0.31522249229981136
...
# Root Mean Squared Error (RMSE): 403813.5917976016
# R-squared (R2) Score: 0.7329409536565354
...
# Root Mean Squared Error (RMSE): 395484.89622649795
# R-squared (R2) Score: 0.7438435870309983
...
# output RMSE and R^2
print(f'Root Mean Squared Error (RMSE): {rm10}')  # Root Mean Squared Error (RMSE): 313732.19802170136
print(f'R-squared (R2) Score: {res10.rsquared}')  # R-squared (R2) Score: 0.7302970537826929

# Compared with linear_model.LinearRegression(), sm.OLS got much better RMSE with minimal R-squared loss
...
# output RMSE and R^2
print(f'Root Mean Squared Error (RMSE): {rm}')  # Root Mean Squared Error (RMSE): 303626.2536832995
print(f'R-squared (R2) Score: {res.rsquared}')  # R-squared (R2) Score: 0.7473925537404007
